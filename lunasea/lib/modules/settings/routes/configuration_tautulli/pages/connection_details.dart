...
class _State extends State<ConfigurationTautulliConnectionDetailsRoute>
  ...
  Widget? _bottomActionBar() {
    if (ServerDrivenConnection.shouldRequestAccess('tautulli')) return null;
    return LunaBottomActionBar(
      actions: [
        if (!ServerDrivenConnection.isManaged('tautulli')) _testConnection(),
        _shareConfiguration(),
      ],
    );
  }
    ...
    return LunaBox.profiles.listenableBuilder(
      builder: (context, _) => LunaListView(
        ...
        children: ServerDrivenConnection.isManaged('tautulli')
            ...
            : ServerDrivenConnection.shouldRequestAccess('tautulli')
  ...
  Widget _testConnection() {
    return LunaButton.text(
      ...
      onTap: () async {
        ...
        final blocked = await SsrfGuard.guard(_profile.tautulliHost);
        if (blocked != null) {
